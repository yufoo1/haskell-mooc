...
totalPrice (MkShoppingEntry s d i) = d * (fromIntegral i)
...
setName nn (MkPerson { name = n, age = a }) = MkPerson { name = nn, age = a }

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge na (MkPerson { name = n, age = a }) = MkPerson { name = n, age = na }
...
up (MkPosition { x = xx, y = yy }) = MkPosition { x = xx, y = yy + 1 }

-- right increases the x value of a position by one
right :: Position -> Position
right (MkPosition { x = xx, y = yy }) = MkPosition { x = xx + 1, y = yy }
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix x y) = map (\x -> x / 2) (zipWith (+) (rgb x) (rgb y))
...
fromList [] = Empty
fromList (l : lx) = Pair (fst l) (snd l) (fromList lx)
...
toNat :: Int -> Maybe Nat
toNat n = if n < 0 then Nothing else Just (f n)
  where f :: Int -> Nat
        f 0 = Zero
        f x = PlusOne (f (x - 1))
