...
instance Eq Name where
  ...
  Name "" == Name xs = False
  Name xs == Name "" = False
  Name (x : xs) == Name (y : ys) = if (toLower x) == (toLower y) then Name xs == Name ys else False
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = RationalNumber (getSign a) (getSign b)
                                  where getSign :: Integer -> Integer 
                                        getSign i = if i > 0 then 1 else if i == 0 then 0 else -1 
